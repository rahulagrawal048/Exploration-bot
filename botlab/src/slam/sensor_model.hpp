...
public:
    ...
    */
    double likelihood(const particle_t& particle, const lidar_t& scan, const OccupancyGrid& map);
    std::vector<Point<int>> get_cell_before(const adjusted_ray_t& ray, const OccupancyGrid& map);
    std::vector<Point<int>> get_cell_after(const adjusted_ray_t& ray, const OccupancyGrid& map);
private:
